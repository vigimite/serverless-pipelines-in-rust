...
pub async fn process_month(input_date: NaiveDate) -> Result<(), DataFusionError> {
    // Load data frame
    ...
    info!("Loading data frame at {}", input_path);
    let df = ctx
...
async fn merge_location_stats(location_stats: DataFrame) -> Result<(), DeltaTableError> {
    info!("Writing location stats table");
